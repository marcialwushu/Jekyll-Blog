...
Em Londres, ônibus e estações de metrô aceitam pagamentos por telefone via NFC, onde você normalmente tocaria no cartão de viagem. Da mesma forma, na China, a NFC é aceita como meio de pagamento em todos os transportes públicos e, no Japão, também está sendo usada para fornecer informações sobre cartões de identidade. Em Nice, visitantes e residentes podem usar a NFC para comprar quase tudo.

A NFC envolve uma transferência direta, quase instantânea, de dados criptografados para dispositivos de  ponto de venda , em oposição à tecnologia de chip e PIN, que leva mais tempo para processar. Muitas operadoras de telefonia móvel estão procurando maneiras de desenvolver mais essa tecnologia.

![](https://www.mobiletransaction.org/wp-content/uploads/2018/05/nfc-payment-card-terminal-1200x800px-800x533.jpg)
...
A transmissão magnética segura é tão segura quanto a NFC, em parte porque ambos usam um sistema de tokenização seguro e é mais seguro do que usar um cartão fisicamente presente.
...
O Google Pay e o Samsung Pay são outros dois grandes concorrentes em carteira móvel.

As carteiras digitais são muito seguras de usar. Geralmente, eles trabalham com criptografia e tokenização complexas, um método que utiliza números de token com tempo limitado gerados para processar a transação específica usando o cartão já criptografado “armazenado” na carteira móvel.
...
Em todos os casos, você deve apontar a câmera com cuidado para corresponder aos indicadores na tela do telefone antes que o aplicativo possa digitalizar o código. Algumas pessoas afirmam que isso é um aborrecimento, principalmente porque você abriu o aplicativo relevante antes de começar a digitalizar.

![](https://www.mobiletransaction.org/wp-content/uploads/2018/07/qr-code-payment-iphone-1200x800px-800x533.jpg)


>Os códigos QR seguem algumas etapas para abrir um aplicativo no telefone e, em seguida, uma quantidade básica de precisão para digitalizar, o que para algumas pessoas pode parecer uma barreira.

Algumas empresas de comércio eletrônico usam códigos QR no checkout do site como uma alternativa para inserir manualmente os detalhes do cartão. Para transações com cartão não presente, isso é mais seguro porque o telefone, com os quais os detalhes do cartão estão conectados com segurança, confirma que você é o proprietário do cartão e porque não está digitando os detalhes do cartão não criptografado na tela do dispositivo.
...
>Como os smartphones podem acessar a Internet completa, é comum as pessoas simplesmente inserirem os detalhes do cartão em uma página de check-out do site para finalizar o pagamento.

Antes de 2010, isso era conhecido como pagamentos por protocolo de aplicativo sem fio (WAP). O WAP costumava ser a instalação mais comum em smartphones conectados à Internet. Portanto, em vez de um navegador da Web com acesso a toda a Internet, as pessoas pagam por meio de um navegador ou aplicativo WAP de capacidade mais limitada - juntos, classificados como pagamentos WAP.
...
![](https://www.mobiletransaction.org/wp-content/uploads/2018/06/mobile-banking-screen-1200x799px-800x533.jpg)


O mobile banking é simplesmente um aplicativo fornecido pelo banco do usuário, através do qual você pode realizar transações financeiras diretamente da sua conta bancária. Isso geralmente é usado para transferências e pagamentos ponto a ponto a outras pessoas, mas as contas também podem ser pagas dessa maneira.

Cada banco tem seus próprios procedimentos de inscrição para o aplicativo para verificar se você é o proprietário da conta bancária. Porém, após a inscrição, geralmente é fácil fazer login no telefone e ver o saldo da conta e o histórico de transações, fazer transferências bancárias e qualquer outra coisa que o banco permita. Todo banco tem seus próprios limites para o que você pode fazer através do aplicativo.


---

Autor: [Emily Sorensen](https://www.mobiletransaction.org/)

[Artigo Original](https://www.mobiletransaction.org/different-types-of-mobile-payments/)
